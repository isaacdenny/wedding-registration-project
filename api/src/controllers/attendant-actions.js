...
export const deleteAttendant = async (req, res) => {
  try {
    ...
    let sql = `DELETE FROM attendants WHERE (uuid='${id}')`;
...
export const updateAttendant = async (req, res) => {
  try {
    const { id, name, partyName, invitationID, isAttending } =
      await req.body;
    let sql = `UPDATE attendants SET name = '${name}', partyName = '${partyName}', invitationID = '${invitationID}', isAttending = ${isAttending} WHERE (uuid='${id}')`;
    let query = db.query(sql, (err, result) => {
